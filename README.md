...
8. PLS_gradients_new_SAPS_SANS --> PLS with only one latent variable run on one gradient at a time. Three models: the first with age and gender regressed only from the brain, the second regressing them out also from behavior, the third without cognition.
9. PLS_all_3_gradients --> PLS using all 3 gradients together and 10 LVs.
